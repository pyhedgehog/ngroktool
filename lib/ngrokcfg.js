...
function enumauth(cb) {
  return cfg.ngroktool.auth.forEach(cb);
}
...
function setauth(auth, checkCB, cb) {
  ...
  if(!auth.name) {
    if(!auth.login) {
      console.log("Error: No auth name given.\n");
      return;
    }
    auth.name = auth.login.split('@')[0];
  }
  auth2 = getauth(auth.name);
  if(auth2 && auth2.login) {
    debuglog('auth =', auth);
    debuglog('auth2 =', auth2);
    auth = Object.assign({}, auth2, Object.keys(auth).reduce(function(o,k){if(auth[k])o[k]=auth[k];return o;},{}));
    debuglog('merged auth =', auth);
  }
  if(!auth.authtype) {
    auth.authtype = 'ngrok';
  }
...
function gettoken() {
  return cfg.ngrok.authtoken;
}

function gettokenauth() {
  var token = gettoken(), auth = null;
  enumauth(function(elem) {
    if(token == elem.authtoken) {
      auth = elem;
    }
  });
  return auth;
...
function getngrokstore(action, args) {
  var res = {};
  res.uri = cfg.ngroktool.ngrokstore || 'https://hook.io/pyhedgehog/ngrokstore';
  args = args || {};
  if(action == 'add') {
    args.setup = cfg.ngroktool.ngrokstorekey;
  }
  args.action = action;
  if(action == 'read') {
    res.qs = args;
